...
class overtime_request(models.Model):
    ...
    debit_account_id = fields.Many2one('account.account', 'Debit Account', tracking=True, readonly=True,
                                       states={'draft': [('readonly', False)]},default=lambda self: self.env.company.req_overtime_acc_debit_id.id)
    credit_account_id = fields.Many2one('account.account', 'Credit Account', tracking=True, readonly=True,
                                        states={'draft': [('readonly', False)]},
                                        default=lambda self: self.env.company.req_overtime_acc_credit_id.id)
    journal_id = fields.Many2one('account.journal', 'Journal', tracking=True, readonly=True,
                                 states={'draft': [('readonly', False)]},
                                 default=lambda self: self.env.company.req_overtime_journal_id.id)
