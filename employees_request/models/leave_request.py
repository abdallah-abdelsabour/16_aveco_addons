...
class leave_request(models.Model):
    ...
    request_date = fields.Date(string="Request Date", required=True,tracking=True, readonly=True, states={'draft': [('readonly', False)]},)
    ...
    leave_entitlement = fields.Selection(string="Leave Entitlement",related="employee_id.leave_entitlement", tracking=True,
                                         selection=[('yearly', 'Yearly'), ('2 years', '2 Years')])

    annual_leave_days = fields.Integer(string="Annual Leave Days",related="employee_id.annual_leave_days", tracking=True,)
    number_of_days_due = fields.Integer(string="Due Days Number",tracking=True,readonly=True, states={'draft': [('readonly', False)]},)
    value_of_days_due = fields.Float(string="Due Days Value" , tracking=True,compute="compute_due_days_value")
    basic_salary = fields.Float(string="Basic Salary", tracking=True, compute="get_basic_salary")

    travel_ticket_value = fields.Float(string="Travel Ticket Value", related="employee_id.travel_ticket_value",readonly=False,
                                       tracking=True, )
    period_of_leave_due = fields.Char(string="period Of leave due",tracking=True,compute="cal_period_of_leave_due")
    note = fields.Text(string="Note",readonly=True, tracking=True,states={'draft': [('readonly', False)]},)
    total_allowance_value = fields.Float(string="Total Allowance Value",tracking=True,compute="compute_allowance_value")
    ...
    debit_account_id = fields.Many2one('account.account', 'Debit Account',tracking=True,readonly=True, states={'draft': [('readonly', False)]},default=lambda self: self.env.company.req_leave_acc_debit_id.id)
    credit_account_id = fields.Many2one('account.account', 'Credit Account',tracking=True,readonly=True, states={'draft': [('readonly', False)]}, default=lambda self: self.env.company.req_leave_acc_credit_id.id)
    journal_id = fields.Many2one('account.journal', 'Journal',tracking=True,readonly=True, states={'draft': [('readonly', False)]},
