...
class end_service(models.Model):
    ...
    alwns_amount = fields.Float(string="Allowances", tracking=True, compute="cal_alwns_amt")
    ...
    debit_account_id = fields.Many2one('account.account','Debit Account',readonly=True, states={'draft': [('readonly', False)]}, default=lambda self: self.env.company.req_endserv_acc_debit_id.id)
    credit_account_id = fields.Many2one('account.account','Credit Account',readonly=True, states={'draft': [('readonly', False)]}, default=lambda self: self.env.company.req_endserv_acc_credit_id.id)
    journal_id = fields.Many2one('account.journal','Account Journal',readonly=True, states={'draft': [('readonly', False)]},default=lambda self: self.env.company.req_endserv_journal_id.id)
